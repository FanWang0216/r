...
check_for_winner <- function(postal_code) {
  
  first_number <- as.numeric(str_sub(postal_code, 2, 2))
  second_number <- as.numeric(str_sub(postal_code, 5, 5))
  third_number <- as.numeric(str_sub(postal_code, 7, 7))
...
```{r}
fix_mistakes <- function(entered_code) {
  
  # Write your code here
  if (str_length(entered_code) == 6) {
    first_part <- str_sub(entered_code, 1, 3)
    second_part <- str_sub(entered_code, 4, 6)
    code_with_space <- str_c(first_part, " ", second_part)
  } else {
    code_with_space <- entered_code
  }

  return(str_to_upper(code_with_space))
}
